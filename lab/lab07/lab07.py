...
def reverse(lst):
    ...
    [-8, 72, 42]
    """
    start, middle, last = 0, len(lst) // 2, len(lst) - 1
    while start < middle:
...
def make_glookup(class_assignments):
    ...
    0.8913043478260869
    """
    total_point, total_score = 0, 0
    def glookup(str, point):
        nonlocal total_point, total_score
        total_score += class_assignments[str]
        total_point += point
        return total_point / total_score
    return glookup
