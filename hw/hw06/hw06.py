...
class VendingMachine:
    ...
    def vend(self):
        ...
        elif self.balance < self.amout:
            return 'You must add ${0} more funds.'.format(self.amout - self.balance)
        elif self.balance == self.amout:
            self.balance = 0
            self.stock -= 1
            return 'Here is your {0}.'.format(self.name)
        else:
            money = self.balance - self.amout
            self.balance = 0
            self.stock -= 1
            return 'Here is your {0} and ${1} change.'.format(self.name, money)
...
def store_digits(n):
    ...
    def helper(lst):
        if len(lst) == 1:
            return Link(lst[0])
        return Link(lst[len(lst) -1], helper(lst[0:len(lst)-1]))
    return helper(lst)
