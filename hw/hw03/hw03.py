...
def g_iter(n):
    ...
    if n <= 3:
        ...
        one, two, three, i, ith = 1, 2, 3, 3, 3
        while i < n:
            ith = three + 2 * two + one * 3
            one, two, three, i = two, three, ith, i + 1
        return ith
...
def missing_digits(n):
    ...
    if n < 10:
        ...
        lose, ones, n = 0, n % 10, n //10
        if ones - n % 10 > 1:
            lose = ones - n % 10 - 1
        return lose + missing_digits(n)
...
def count_change(total):
    ...
    True
    """
    def count_with_factor(total, factor):
        if total == 0:
            return 1
        if total < factor:
            return 0
        return count_with_factor(total - factor, factor) + count_with_factor(total, factor * 2)
    return count_with_factor(total, 1)
