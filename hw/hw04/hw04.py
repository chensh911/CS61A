...
def balanced(m):
    ...
    if length(left(m)) * total_weight(left_side) != length(right(m)) * total_weight(right_side):
        return False
    return balanced(left_side) and balanced(right_side)
...
def replace_leaf(t, find_value, replace_value):
    ...
    return tree(label(t),[replace_leaf(a, find_value, replace_value) for a in branches(t)])
